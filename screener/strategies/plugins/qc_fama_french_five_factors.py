...
def _prepare_ff5(ctx: PrepareCtx) -> dict[str, pd.DataFrame]:
    ...
    for sym, bars in ctx.bars_by_tv.items():
        ...
        df["ff5_long"] = long_daily.get(sym, pd.Series(False, index=df.index)).shift(1).fillna(False).astype(int)
        df["ff5_short"] = short_daily.get(sym, pd.Series(False, index=df.index)).shift(1).fillna(False).astype(int)
